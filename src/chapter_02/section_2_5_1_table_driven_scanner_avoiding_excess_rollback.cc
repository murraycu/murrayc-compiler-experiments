 ...
 *
 * Based on the pseudocode in Figure 2.15, in section 2.5.1,
...
scan(const std::string& str, const std::set<T_StateID>& states, const T_StateID& s0, const T_CharCat& CharCat, const T_Delta& Delta, const T_Type& Type, std::set<std::pair<T_StateID, std::size_t>>& failed) {
  ...
  const auto nstr = str.size();
  std::size_t input_pos = 0;

  using StateAndPos = std::pair<T_StateID, std::size_t>;
  std::stack<StateAndPos> st;
  st.emplace(STATE_BAD, 0);

  while (state != STATE_END) {
    const auto ch = (input_pos >= nstr ? CHAR_EOF : str[input_pos]);
    ++input_pos;

    lexeme += ch;

    if (failed.count(std::make_pair(state, input_pos))) {
      break;
    }
...
// TODO: Build tables for the ab|(ab)*c regex DFA mentioned in the text.
...
recognise_register(const std::string& str) {
  ...
  // Set of failed state/input-pos combinations,
